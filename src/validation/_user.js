...
import { ID_MAX_LENGTH, ID_MIN_LENGTH, PW_MAX_LENGTH, PW_MIN_LENGTH } from "../constants/validation.constants";
...
export const userPwSchema = z.object({
    pw: z.string("비밀번호는 반드시 문자열이어야 합니다.")
        ...
        .max(PW_MAX_LENGTH, `비밀번호는 최대 ${PW_MAX_LENGTH}자를 넘을 수 없습니다.`)
...
export const idValidCheck = (id) => {
    ...
    if(idValidCheckResult.success === false) {
        return {success: false, message: extractZodMessage(idValidCheckResult.error)};
    }
...
export const pwValidCheck = (pw) => {
    ...
    if(pwValidCheckResult.success === false) {
        return {success: false, message: extractZodMessage(pwValidCheckResult.error)};
    }
