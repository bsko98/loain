...
const PartyContainer = ({ partyData, selectedFilters }) => {
  ...
  const { boss, difficulty, startTime, environment } = partyData;

  const partyFilter = partyData.partyFilter;

  const partytitle = partyData.title;
  const member = partyData.partyMembers.filter(
    member => member !== null
  ).length;
  ...
  const isTagMatched = (tagName, value) => {
    ...
    if (tagName === 'isLastPot' || tagName === 'isLastDeal') {
      return (
        (selectedFilters.isLastPot && value === true) ||
        (selectedFilters.isLastDeal && value === true)
      );
    }
  ...
  // 태그 목록 (빈 값 제외)
  const allTags = [
    partyFilter.itemLevel && {
      label: `#${partyFilter.itemLevel}↑`,
      key: 'itemLevel',
      value: partyFilter.itemLevel,
    },
    partyFilter.skillRange && {
      label: `#${partyFilter.skillRange}`,
      key: 'skillRange',
      value: partyFilter.skillRange,
    },
    partyFilter.title && {
      label: `#${partyFilter.title}`,
      key: 'title',
      value: partyFilter.title,
    },
    partyFilter.card[0].name &&
      partyFilter.card[0].awakening && {
        label: `#${partyFilter.card[0].name} : ${partyFilter.card[0].awakening}`,
        key: 'card',
        value: partyFilter.card[0].name,
      },
    environment && {
      label: `#${environment}`,
      key: 'environment',
      value: environment,
    }, //예민 x 예민 o 처리부분인데 아직 데이터가 없음. 나중에 추가
    partyFilter.arkPassive.evolution && {
      label: `#진화 ${partyFilter.arkPassive.evolution}`,
      key: 'evolution',
      value: partyFilter.arkPassive.evolution,
    },
    partyFilter.arkPassive.enlightenment && {
      label: `#깨달음 ${partyFilter.arkPassive.enlightenment}`,
      key: 'realization',
      value: partyFilter.arkPassive.enlightenment,
    },
    partyFilter.arkPassive.leap && {
      label: `#도약 ${partyFilter.arkPassive.leap}`,
      key: 'leap',
      value: partyFilter.arkPassive.leap,
    },
    partyFilter.transcend.weapon && {
      label: `#무기 초월 ${partyFilter.transcend.weapon}`,
      key: 'transcendenceWeapon',
      value: partyFilter.transcend.weapon,
    },
    partyFilter.transcend.armor && {
      label: `#방어구 초월 ${partyFilter.transcend.armor}`,
      key: 'transcendenceArmor',
      value: partyFilter.transcend.armor,
    },
    partyFilter.lastDealer && {
      label: '#랏딜',
      key: 'lastDealer',
      value: partyFilter.lastDealer,
    },
    partyFilter.lastSupporter && {
      label: '#랏폿',
      key: 'lastSupporter',
      value: partyFilter.lastSupporter,
    },
      ...
      <div className="party-container-main">
        <div className="party-container-left">
          ...
          <div className="party-container-info-tags">
            <span>#{boss}</span> |<span>#{difficulty}</span> |
              ...
              <div className="extra-tag-line" key={groupIdx}>
                {tagGroup.map((tag, idx) => (
                  <span
                    key={idx}
                    className={
                      isTagMatched(tag.key, tag.value) ? 'highlight-tag' : ''
                    }
                  >
                    {tag.label}
                  </span>
        ...
        <div className="party-container-right">
          <div className="party-container-top">
            <div className="party-container-member-count">
              {member}/{partyData.partyMembers.length}
            </div>
