...
const Sidebar = ({ state }) => {
  ...
  const loggedInItems = [
    ...
    {
      id: 3,
      url: '/manageMyCharacter',
      name: '내 캐릭터 관리',
      Component: ManageMyCharacter,
      blueComponent: BlueManageMyCharacter,
    },
    {
      id: 4,
      url: '/myParty',
  ...
  return (
    <div className="side-bar" style={{ fontFamily: 'Pretendard' }}>
      ...
      {moreInfo ? (
        <div className="more-item-container">
          <div className="more-item-box">
            {displayMoreList.map(item => (
              <li
                key={item.id}
                className="more-item-box-row"
                onClick={() => openManageMyAccountModal(item)}
              >
                {' '}
                <item.Component style={{ marginRight: '4px' }} />
                <div
                  style={{
                    fontSize: '15px',
                    color: '#DFDFDF',
            ...
            <div
              className={
                state.isLoggedIn ? 'logged-in-terms-of-use' : 'terms-of-use'
              }
            >
              <div style={{ cursor: 'pointer' }}>이용약관</div>
              <div style={{ marginLeft: '3px', marginRight: '3px' }}>⋅</div>
              <div style={{ cursor: 'pointer' }}>개인정보 처리방침</div>
            </div>
          </div>
        </div>
      ) : null}
      <div>
        <div className="more-info-button">
          <More
            ...
            onClick={() => activeMoreInfo(moreInfo)}
          />
          <div
            style={{
              userSelect: 'none',
      ...
      <Signin
        ...
        isFindIdOpen={() => setIsFindIdModalOpen(!isFindIdModalOpen)}
        isResetPasswordOpen={() =>
          setIsResetPasswordModalOpen(!isResetPasswordModalOpen)
        }
        isSignUpOpen={() => setIsSingUpModalOpen(!isSingUpModalOpen)}
        setIsLoggedIn={state.setIsLoggedIn}
      />
      <MakePartyModal
        isOpen={isPartyModalOpen}
        onClose={closePartyModal}
        modalTitleText={'파티 만들기'}
        buttonText={'파티 만들기'}
      />
      <ManageMyAccountModal
        isOpen={isMyAccountModalOpen}
        onClose={closeMyAccountModal}
      />
      <ResetPassword
        isOpen={isResetPasswordModalOpen}
        onClose={() => setIsResetPasswordModalOpen(!isResetPasswordModalOpen)}
      />
      <FindId
        isOpen={isFindIdModalOpen}
        onClose={() => setIsFindIdModalOpen(!isFindIdModalOpen)}
      />
      <SignUp
        isOpen={isSingUpModalOpen}
        onClose={() => setIsSingUpModalOpen(!isSingUpModalOpen)}
        setIsLoggedIn={state.setIsLoggedIn}
      />{' '}
      {/*회원가입 부분은 onClose가 없다*/}
    </div>
  );
};
