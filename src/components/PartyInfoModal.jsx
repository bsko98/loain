...
import { UpdatePartySchema } from "../validation/_updateParty.js";
...
const PartyInfoModal = ({
  ...
  const [startMastery, setStartMastery] = useState(
    myParty.partyFilter.startMastery
  );
  ...
  const [filters, setFilters] = useState({
    ...
    title: myParty.partyFilter.title ? myParty.partyFilter.title : undefined,
    card: myParty.partyFilter.card.length
      ? myParty.partyFilter.card[0].name
      : undefined,
    awakening: myParty.partyFilter.card.length
      ? myParty.partyFilter.card[0].awakening
      : undefined,
  ...
  const checkValues = (partyFilter) => {
    ...
    const result = UpdatePartySchema.safeParse(updatedPartyInfo);
    if (result.success) {
      return true;
    } else {
      console.log(result);
      alert("문제가 발생했습니다. 입력하신 내용을 다시 확인해주세요");
      return false;
    }
  };
