  ...
  return (
    <div className="my-character-container">
      <div className="my-character-add-button-row">
        ...
        <button
          className="open-add-character-button"
          onClick={() => setIsAddCharacterModalOpen(!isAddCharacterModalOpen)}
        >
          <AddButton />
          <span className="add-character-button-text">추가하기</span>
        </button>
      </div>
      ...
      <div className="my-character-info-box">
        {state.myData.characters.length > 0 ? (
          state.myData.characters
            .filter(
              char => activeServer === null || char.serverName === activeServer
            )
            .map(character => (
              <CharacterInfoComponent
                ...
                comp={
                  <button
                    className="refresh-character-info-button"
                    onClick={() => updateCharacter(character.name)}
                    disabled={state.refreshCooldown}
                  >
                    정보 갱신
                  </button>
                }
