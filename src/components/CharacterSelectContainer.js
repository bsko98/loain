...
const CharacterSelectContainer = ({ state }) => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      state.setMyData(
        selectCharacter(state.myData, state.myData.characters[0])
      );
    } catch (error) {
  ...
  return (
    <div className="character-select-container">
        ...
        {state.myData.userData.chooseCharacter ? (
          <>
            <span className="character-name">
              {state.myData.userData.chooseCharacter.name}
            </span>
          </>
        ) : (
      ...
      <div className="character-select-button-group">
        <button
          className="character-select-btn refresh"
        ...
        <button
          className="character-select-btn change"
          onClick={() => setIsModalOpen(true)}
        >
          <img
            src={CharacterSelectIcon}
            alt="캐릭터 변경"
            className="character-select-icon"
          />
          캐릭터 선택
        </button>
      </div>
