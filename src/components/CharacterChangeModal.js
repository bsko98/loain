...
const CharacterChangeModal = ({
  ...
  const filterByServer = server => {
    setFilteredCharacters(
      characterList.filter(char => char.serverName === server)
    );
    ...
    <div className="character-change-modal-overlay">
      <div className="character-change-modal-content" ref={modalRef}>
        <div className="character-change-modal-header">
          <h2>내 캐릭터</h2>
          <button
            className="character-change-modal-close-btn"
            onClick={onClose}
          >
            <CloseButton />
          </button>
                ...
                <CharacterInfoComponent
                  ...
                  onclick={() =>
                    onSelectCharacter(
                      ...
                      state.myData.userData
                    )
                  }
                  comp={
                    selectedCharacter?.characterId ===
                      character.characterId && (
                      <span className="character-change-selected-label">
                        선택됨
                      </span>
                    )
                  }
