...
export class PartyEntityMapper {
  static toInternal(party) {
    return {
      ...
      partyMembers: party.partyMembers.map(member => {
        return member === null ? null : UserDataEntityMapper.toInternal(member);
      }),
      allow: party.allow,
      volunteers: party.volunteers.map(volunteers =>
        UserDataEntityMapper.toInternal(volunteers)
      ),
    };
  }
}
